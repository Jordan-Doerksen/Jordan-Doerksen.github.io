...
<title>The trading desk, one path at a time</title>
...
.what-it-is{margin:0 0 22px;max-width:60ch;font-size:16px;line-height:1.6;color:var(--ink-2)}
.what-it-is b{color:var(--ink);font-weight:600}
...
/* PAGE-WIDE, stacked. Three across put a three-column trading cockpit and a
   signal head with a full indication sentence into ~300px each: cramped, and
   the signal caption overlapped its own footer. These demos are detailed
   interfaces, not thumbnails - they need the width to be legible at all. */
.bench-wall{display:grid;grid-template-columns:1fr;gap:clamp(18px,2.4vw,30px)}
.bench-wall .panel{border:1px solid var(--hair);background:var(--sheet)}
.bench-wall .screen{position:relative;aspect-ratio:16/9;background:#04060B;overflow:hidden;
  border-bottom:1px solid var(--hair)}
@media (max-width:700px){.bench-wall .screen{aspect-ratio:4/3}}
.bench-wall .screen iframe{position:absolute;inset:0;width:100%;height:100%;border:0;display:block}
.bench-wall .meta{padding:15px 18px 18px}
.bench-wall h3{margin:0 0 6px;font:600 17px/1.3 var(--sans)}
.bench-wall h3 a{color:var(--ink);text-decoration:none}
.bench-wall h3 a:hover,.bench-wall h3 a:focus-visible{color:var(--live)}
.bench-wall p{margin:0;color:var(--ink-2);font-size:14.5px;max-width:70ch}
...
<div class="wrap">
  ...
  <h1>The trading desk, one path at a time.</h1>
  <!-- The section used to open on "the desk" and never say what that was. A
       reader arriving here has no idea whether it trades, and it does not. -->
  <p class="what-it-is">A read-only market desk. It records the tape, measures it, and
  reports what it sees. <b>It never places an order</b> — that is a constraint in the
  build, not a setting. Below is every path data takes through it, one part at a time.</p>
  <p class="idea">@@idea@@</p>
